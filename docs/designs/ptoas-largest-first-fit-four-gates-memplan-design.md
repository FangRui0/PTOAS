...
pto.tfillpad {mode = #pto.tfillpad_mode<expand>}
...
其中 `pto.tands` / `pto.tors` / `pto.txors` 和 expand 模式的 `pto.tfillpad` 是 PTOAS 侧额外保守标记的 non-inplace-safe op。它们虽然不是 scratch-output conflict，但后端/ISA 语义没有明确承诺 input/output alias 安全，memplan 不应通过地址复用隐式把它们变成 inplace 执行。

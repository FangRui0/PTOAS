...
# compilation of the `ptoas.mlir` pybind11 extension family (_mlir core,
# _mlirDialectsLLVM, _site_initialize_0).
#
...
reg_cpp="$(cd "$script_dir/../../.." && pwd)/lib/Bindings/Python/PythonRegistration.cpp"
if [[ -f "$reg_cpp" ]]; then
  sources+=("$reg_cpp")
fi
...
{
  echo "# LLVM/MLIR C++ header closure for online-compiling the ptoas.mlir"
  echo "# pybind11 extension family. GENERATED by gen_mlir_family_headers.sh --"

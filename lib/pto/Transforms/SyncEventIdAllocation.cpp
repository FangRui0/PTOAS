...
size_t SyncEventIdAllocation::GetCompilerAvailableEventIdNum(const SyncOperation *sync) {
  ...
  return kTotalEventIdNum -
         getReservedEventIdNum(sync->GetSrcPipe(), sync->GetDstPipe());
}

uint64_t SyncEventIdAllocation::getReservedEventIdNum(PipelineType srcPipe,
                                                      PipelineType dstPipe) {
  if ((srcPipe == PipelineType::PIPE_V && dstPipe == PipelineType::PIPE_S) ||
      (srcPipe == PipelineType::PIPE_S && dstPipe == PipelineType::PIPE_V) ||
      (srcPipe == PipelineType::PIPE_MTE2 &&
       dstPipe == PipelineType::PIPE_V)) {
    return 1;
  }
  return 0;
}
...
// (No global maps here: keep this TU free of global constructors.)

...
LogicalResult mlir::pto::TransOp::inferReturnTypes(
  ...
  auto vr = dyn_cast_or_null<DenseI64ArrayAttr>(attributes.get("valid_range"));
...
LogicalResult pto::MGatherOp::verify() {
  ...
  if (!isa<IntegerType>(idxElem) && !isa<IndexType>(idxElem))
...
LogicalResult pto::AbsOp_DPS::verify() {
  ...
  bool ok = false;
  if (auto it = dyn_cast<IntegerType>(se)) {
...
LogicalResult pto::AndSOp_DPS::verify() {
  ...
  Type ed = md.getElementType();
  if (!isa<IntegerType>(es) || !isa<IntegerType>(ed))
...
LogicalResult pto::AssignOp_DPS::verify() {
  ...
  if (!isa<IndexType>(addrTy) && !isa<IntegerType>(addrTy))
...
LogicalResult pto::CIOp_DPS::verify() {
  ...
  auto elemTy = dyn_cast<IntegerType>(dstTy.getElementType());
  if (!elemTy)
  ...
  auto sTy = dyn_cast<IntegerType>(getS().getType());
  if (!sTy)
...
mlir::LogicalResult mlir::pto::GatherbOp_DPS::verify() {
  auto srcTy = dyn_cast<mlir::MemRefType>(getSrc().getType());
  auto offTy = dyn_cast<mlir::MemRefType>(getOffsets().getType());
  auto dstTy = dyn_cast<mlir::MemRefType>(getDst().getType());
  ...
  auto elemBits = [](mlir::Type t) -> std::optional<unsigned> {
    if (auto i = dyn_cast<mlir::IntegerType>(t))
      return i.getWidth();
    if (auto f = dyn_cast<mlir::FloatType>(t))
      return f.getWidth();
    return std::nullopt;
  };
...
mlir::LogicalResult mlir::pto::SqrtOp_DPS::verify() {
  ...
  if (!(srcElem.isF32() || srcElem.isF16()))
...
static bool isPTOShapedLike(Type ty) {
  return isa<MemRefType, RankedTensorType,
                pto::TileBufType, pto::TileViewType>(ty);
}

static Type getElemTy(Type ty) {
  if (auto mr = dyn_cast<MemRefType>(ty)) return mr.getElementType();
  if (auto tt = dyn_cast<RankedTensorType>(ty)) return tt.getElementType();
  if (auto tb = dyn_cast<pto::TileBufType>(ty)) return tb.getElementType();
  if (auto tv = dyn_cast<pto::TileViewType>(ty)) return tv.getElementType();
  return Type();
}
...
static SmallVector<int64_t, 4> getShapeVec(Type ty) {
  ...
  if (auto tt = dyn_cast<RankedTensorType>(ty))
  ...
  if (auto tv = dyn_cast<pto::TileViewType>(ty))
...
mlir::LogicalResult mlir::pto::TGatherbOp::verify() {
  auto srcTy = dyn_cast<mlir::pto::TileBufType>(getSrc().getType());
  auto offTy = dyn_cast<mlir::pto::TileBufType>(getOffsets().getType());
  auto dstTy = dyn_cast<mlir::pto::TileBufType>(getDst().getType());
  if (!srcTy || !offTy || !dstTy)
...
static void decomposeStridedLayout(AffineMap map, SmallVectorImpl<int64_t> &strides) {
  ...
  for (auto term : terms) {
    ...
    if (auto mul = dyn_cast<AffineBinaryOpExpr>(term)) {
    ...
    else if (auto dim = dyn_cast<AffineDimExpr>(term)) {

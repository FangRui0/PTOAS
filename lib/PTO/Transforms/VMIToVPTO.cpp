...
struct OneToNVMIExtIOpPattern : OneToNOpConversionPattern<OpT> {
  ...
  matchAndRewrite(OpT op,
    ...
    // A dense integer extension can use unpack directly when the assigned
    // layouts preserve the physical lane order.  For example, the
    // bf16->ui16->ui32 sequence in per-token cast is deinterleaved=2 to
    // deinterleaved=4: each source chunk produces one result chunk, while
    // the result layout interleaves the source parts by unpack selector.
    if (sourceLayout && resultLayout &&
        ((sourceLayout.isContiguous() &&
          (resultLayout.isContiguous() || resultLayout.isDenseSplit())) ||
         (sourceLayout.isDenseSplit() && resultLayout.isDenseSplit())) &&
        sourceLayout.getLaneStride() == 1 &&
        resultLayout.getLaneStride() == 1 && sourceBits > 0 &&
        resultBits > sourceBits && resultBits % sourceBits == 0 &&
        (resultBits / sourceBits == 2 || resultBits / sourceBits == 4)) {
      int64_t widenFactor = resultBits / sourceBits;
      FailureOr<int64_t> sourceFactor =
          getVMITypeLayoutFactor(sourceVMIType);
      FailureOr<int64_t> resultFactor =
          getVMITypeLayoutFactor(resultVMIType);
      bool contiguousLayouts = sourceLayout.isContiguous() &&
                               resultLayout.isContiguous();
      bool splitLayouts = sourceLayout.isDenseSplit() &&
                          resultLayout.isDenseSplit();
      bool contiguousToSplit = sourceLayout.isContiguous() &&
                               resultLayout.isDenseSplit();
      bool compatibleFactors =
          succeeded(sourceFactor) && succeeded(resultFactor) &&
          ((contiguousLayouts && *sourceFactor == *resultFactor) ||
           ((splitLayouts || contiguousToSplit) &&
            *resultFactor == *sourceFactor * widenFactor));
      if (compatibleFactors) {
        bool deinterleaveBeforeUnpack =
            splitLayouts && *sourceFactor == 2 && *resultFactor == 4 &&
            widenFactor == 2 && sourceParts.size() == 2;
        SmallVector<Value> extensionSources(sourceParts);
        if (deinterleaveBeforeUnpack) {
          auto deinterleaved = rewriter.create<VdintlvOp>(
              op.getLoc(), sourceParts[0].getType(), sourceParts[1].getType(),
              sourceParts[0], sourceParts[1]);
          extensionSources = {deinterleaved.getLow(),
                              deinterleaved.getHigh()};
        }

        SmallVector<SmallVector<Value>> unpacked;
        unpacked.reserve(extensionSources.size());
        for (Value sourcePart : extensionSources) {
          SmallVector<Value> currentValues{sourcePart};
          unsigned currentBits = sourceBits;
          while (currentBits < resultBits) {
            SmallVector<Value> nextValues;
            nextValues.reserve(currentValues.size() * 2);
            unsigned nextBits = currentBits * 2;
            auto nextElementType = IntegerType::get(
                rewriter.getContext(), nextBits,
                cast<IntegerType>(sourceType.getElementType()).getSignedness());
            FailureOr<int64_t> nextLanes =
                getDataLanesPerPart(nextElementType);
            if (failed(nextLanes)) {
              return rewriter.notifyMatchFailure(
                  op, "failed to derive dense integer unpack result lanes");
            }
            auto nextType = VRegType::get(rewriter.getContext(), *nextLanes,
                                          nextElementType);
            for (Value current : currentValues) {
              auto currentType = dyn_cast<VRegType>(current.getType());
              if (!currentType ||
                  currentType.getElementCount() != *nextLanes * 2) {
                return rewriter.notifyMatchFailure(
                    op, "dense integer unpack source/result lane mismatch");
              }
              for (int64_t part = 0; part < 2; ++part) {
                Value partValue = rewriter.create<arith::ConstantIndexOp>(
                    op.getLoc(), part);
                if constexpr (std::is_same_v<OpT, VMIExtSIOp>) {
            ...
            currentValues = std::move(nextValues);
            currentBits = nextBits;
          }
          unpacked.push_back(std::move(currentValues));
        }

        SmallVector<Value> results;
        results.reserve(resultTypes.size());
        if (contiguousLayouts) {
          for (auto [sourceIndex, values] : llvm::enumerate(unpacked)) {
            (void)sourceIndex;
            for (Value value : values) {
              results.push_back(value);
            }
          }
        } else if (deinterleaveBeforeUnpack) {
          for (ArrayRef<Value> values : unpacked)
            results.append(values.begin(), values.end());
        } else {
          bool sourceArityMatches =
              *sourceFactor > 0 &&
              sourceParts.size() == static_cast<size_t>(*sourceFactor);
          if (!sourceArityMatches) {
            return rewriter.notifyMatchFailure(
                op, "dense split integer unpack source arity mismatch");
          }
          for (int64_t resultPart = 0; resultPart < *resultFactor;
               ++resultPart) {
            int64_t sourceIndex = resultPart % *sourceFactor;
            int64_t unpackPart = resultPart / *sourceFactor;
            if (unpackPart < 0 ||
                unpackPart >=
                    static_cast<int64_t>(unpacked[sourceIndex].size())) {
              return rewriter.notifyMatchFailure(
                  op, "dense split integer unpack result arity mismatch");
            }
            results.push_back(unpacked[sourceIndex][unpackPart]);
          }
        }
        bool resultArityMatches = results.size() == resultTypes.size();
        if (!resultArityMatches) {
          return rewriter.notifyMatchFailure(
              op, "dense integer unpack result arity mismatch");
        }
        for (auto [value, resultType] :
             llvm::zip_equal(results, resultVRegTypes)) {
          bool resultTypeMatches =
              cast<VRegType>(value.getType()).getElementCount() ==
                  resultType.getElementCount() &&
              pto::getPTOStorageElemBitWidth(
                  cast<VRegType>(value.getType()).getElementType()) ==
                  resultBits;
          if (!resultTypeMatches) {
            return rewriter.notifyMatchFailure(
                op, "dense integer unpack result type mismatch");
          }
        }
        replaceOpWithFlatConvertedValues(rewriter, op, results,
                                         *this->getTypeConverter());

...
//===- PTOPlanMemory.cpp - static local memory planner --------------------===//
...
static LogicalResult runPlanMemory(func::FuncOp func,
...
namespace {
struct PlanMemoryPass
    : public PassWrapper<PlanMemoryPass, OperationPass<ModuleOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(PlanMemoryPass)

  PlanMemoryPass() = default;
  explicit PlanMemoryPass(const PlanMemoryOptions &options)
      : memMode(options.memMode), orderBySize(options.orderBySize) {}

  StringRef getArgument() const final { return "pto-plan-memory"; }
  StringRef getDescription() const final {
    return "Plan local memory for PTO ops";
  }
  StringRef getName() const final { return "PlanMemory"; }
  ...
  void runOnOperation() override {
    ...
    for (func::FuncOp funcOp : funcs) {
      if (failed(
              runPlanMemory(funcOp, memMode, orderBySize))) {
        signalPassFailure();
        return;
      }
    }

...
ParseResult mlir::pto::TXorOp::parse(OpAsmParser &parser, OperationState &result) {
  OpAsmParser::UnresolvedOperand src0, src1, tmp, dst;
  Type src0Ty, src1Ty, tmpTy, dstTy;
  bool hasTmp = false;

  // Parse: ins(%src0, %src1 : type, type) or ins(%src0, %src1, %tmp : type, type, type)
  if (parser.parseKeyword("ins") || parser.parseLParen() ||
...
mlir::LogicalResult mlir::pto::TXorOp::verify() {
  ...
  auto verifyA2A3 = [&]() -> LogicalResult {
    ...
    if (failed(elemOr))
      return failure();
    if (!getTmp())
      return emitOpError("expects tmp for A2/A3 txor");
    Type tmpTy = getTmp().getType();

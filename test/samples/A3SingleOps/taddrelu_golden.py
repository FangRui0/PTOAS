...
src0 = rng().uniform(-2.0, 2.0, meta.elem_counts[src0_name]).astype(np.float32)
src1 = rng().uniform(-2.0, 2.0, meta.elem_counts[src1_name]).astype(np.float32)
buffers = default_buffers(meta)
buffers.update({src0_name: src0, src1_name: src1})
write_buffers(meta, buffers)
write_golden(meta, {out_name: np.maximum(src0 + src1, np.float32(0.0))})
